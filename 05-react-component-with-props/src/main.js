...
const reactDomRoot = createRoot(container); // { mount, unmount }

// 컴포넌트란? 재사용 메커니즘(클래스 또는 함수)을 사용해 리액트 엘리먼트를 반환하는 것을 말합니다.
// const list = h("ul", { className: "architectures", lang: "en" }, ...children);

// 1. 리액트 클래스 컴포넌트 (재사용 목적)
// 리액트 컴포넌트는 반드시 파스칼케이스(ParscalCase) 유형으로 이름 작성
// 웹 컴포넌트 반드시 이름을 케밥케이스 유형으로 작성 (예: <euid-list></euid-list>)
